...
bool BST_Delete (BST_TREE* tree, void* dltKey, void (*destroyData) (void *data))
{
	...
	newRoot = _delete (tree, tree->root, dltKey, &success, destroyData);
	if (success){
	    tree->root = newRoot;
	    (tree->count)--;
...
static NODE*  _delete (BST_TREE* tree,    NODE* root, 
                void*     dataPtr, bool* success, void (*destroyData) (void *data))
{
	NODE* dltPtr;
	NODE* exchPtr;
	NODE* newRoot;
	             ...
	             while (exchPtr->right)
	              ...
	              root->left       = 
	                 _delete (tree,   root->left, 
	                          exchPtr->dataPtr, success, destroyData);
			 }// else 
	}// node found 
	return root; 
}// _delete 
...
*/
void* BST_Retrieve  (BST_TREE* tree, NODE* root, void* dataPtr)
{
	if (root)
	    return _retrieve (tree, dataPtr, root);
    return NULL;
}// BST_Retrieve 
...
*/
BST_TREE* BST_Destroy (BST_TREE* tree, void (*destroyData) (void *data), int flag) 
{
	if (tree)
		_destroy (tree->root, destroyData, flag);
...
/*	=============== _destroy ============== 
	...
	function to traverse the tree in inorder sequence.
	   ...
	   Return   null head pointer 
*/
static void _destroy (NODE* root, void (*destroyData) (void *data), int flag)
{
	if (root){
	    _destroy (root->left, destroyData, flag);
		if ( flag == 1 ){
		destroyData(root->dataPtr);
	    free (root->dataPtr);
		}
	    _destroy (root->right, destroyData, flag);
	    free (root);
	} 
	return;
}// _destroy
...
//====================== breadthTraversal =======================
/*This function traverse the tree in breadth first order
    pre: A pointer to the root and a pointer to the printData function
    post: Traverse the tree in breadth first order and print data
*/
//void BST_BreadthTraversal(BST_TREE *tree, void (*printData) ( void *dataPtr ))
//{
//    //Local definition
//	NODE*    root;
//	QUEUE*   myQueue;
